...
| One session fires `UserPromptSubmit` but never `Stop` (or vice versa) while other sessions work fine | Claude Code (confirmed on 2.1.141) appears to capture hook script content or hashes at session start. Later edits to the same script can cause selective hook failures in that older session, even though `/hooks` shows the registration count as `(1)`. | Always restart Claude sessions after editing any hook script. Kill the affected session and start a fresh `claude` (without `--resume`). New sessions pick up the current script content cleanly. |
